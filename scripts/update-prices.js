...
// Uso:
//   node scripts/update-prices.js [archivo.xlsx]
//
// Lee AMBAS columnas del Excel:
//   - Columna IZQUIERDA → variant 'unidad'
//   - Columna DERECHA   → variant 'bulto'
//   - Sección GRANEL    → variant 'granel'
//
...
// Se leen AMBAS columnas siempre.
// Sección GRANEL → variant='granel'
...
for (const row of grid) {
  ...
  // Lectura de fila — siempre lee ambas columnas
...
const existingCodes = new Map(); // code -> variant
let updated = 0;
let kept = 0;
...
source = source.split('\n').map(line => {
  ...
  if (!m) return line;
  const code = m[1];
  // Extraer variant del producto existente
  const vm = line.match(/variant:\s*'([^']+)'/);
  const variant = vm ? vm[1] : 'unknown';
...
if (newCodes.length > 0) {
  ...
  const insertion = `\n${banner}${block}\n`;
  // Insert before the LAST ];  (products array), not the first (CATEGORIES)
  const lastClose = source.lastIndexOf('\n];');
  if (lastClose !== -1) {
    source = source.slice(0, lastClose) + insertion + source.slice(lastClose);
  }
}
...
// ─── 4. Reporte ─────────────────────────────────────────────────────
const notInExcel = [...existingCodes.entries()]
  .filter(([code]) => !excelEntries.has(code))
  .map(([code]) => code);
